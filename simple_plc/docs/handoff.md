...
## 5.5. VẤN ĐỀ ĐANG DEBUG — USB enum fail khi board đã chạy sẵn trước lúc cắm cáp

**Triệu chứng thật, xác nhận bởi người dùng trên board thật** (Zigbee-IO
SKU, `board_zigbee_io.c`):

| # | Kịch bản | Kết quả |
|---|---|---|
| 1 | Cấp nguồn qua chính cáp USB (VBUS có ngay từ đầu, MCU boot cùng lúc VBUS lên) | Enum OK |
| 2 | Cấp 3.3V qua ST-Link trước (MCU chạy, chưa có VBUS) → sau đó mới cắm USB | Enum FAIL |
| 3 | Giống #2, nhưng sau đó RESET board (USB vẫn đang cắm) | Enum OK |
| 4 | Rút USB ra (bất kỳ trạng thái nào) | KHÔNG có log "USB Unmounted"/"USB Suspended" nào cả — im lặng hoàn toàn |

**Giả thuyết ĐÃ BỊ BÁC BỎ bởi người dùng, dựa trên kinh nghiệm thực tế
(không phải bằng chứng code) — ghi lại để không lặp lại việc điều tra
sai hướng:** Claude nghi ngờ `hpcd_USB_DRD_FS.Init.vbus_sensing_enable =
DISABLE` (trong `Core/Src/usb.c`, CubeMX tự sinh) là nguyên nhân — lý
thuyết chuẩn về VBUS sensing trên USB device khớp khá tốt với cả 4 triệu
chứng (đặc biệt triệu chứng #4). NHƯNG người dùng xác nhận: **các sản
phẩm STM32 trước đó của họ cấu hình `vbus_sensing_enable = DISABLE` y
hệt (mặc định CubeMX cho `USB_DRD_FS`/`OTG_FS` khi không route chân VBUS
sensing) và vẫn hoạt động bình thường, không có triệu chứng này** — nên
đây khó có thể là nguyên nhân riêng cho project này. Bài học: lý thuyết
khớp triệu chứng không đủ để kết luận, cần đối chiếu với kinh nghiệm/
so sánh thực tế trước khi khẳng định nguyên nhân, đặc biệt trước khi đề
xuất sửa 1 file CubeMX tự sinh (xem mục 6 bài học nghiêm trọng trước đó
về việc không tự sửa file CubeMX sinh — ở đây thậm chí không tới bước
đó vì giả thuyết đã bị bác bỏ trước khi kịp đề xuất sửa cụ thể).

**Đang debug bằng ST-LINK GDB (cortex-debug, VS Code)** — xem
`.vscode/launch.json`. Đã sửa 2 lỗi cấu hình debug thật (không liên
quan bug USB, chỉ là môi trường debug chưa từng chạy được lần nào):
1. `executable` trỏ `build/RS485_IO_RF_V2.elf`, thiếu thư mục con
   `release/` mà `build.bat build` (Release) thực sự tạo ra
   (`build/release/RS485_IO_RF_V2.elf`, xem mục "build.bat" trước đó).
2. `serverpath`/`gdbPath`/`stm32cubeprogrammer` đều trỏ
   `D:\stm32cubeclt\STM32CubeCLT_1.21.0\...` — **ổ đĩa D: không tồn tại
   trên máy người dùng** (`PowerShell` báo "A drive with the name 'D'
   does not exist" khi thử `dir` trực tiếp). Đường dẫn thật, xác nhận
   bằng `Get-Command arm-none-eabi-gdb.exe`:
   `C:\ST\STM32CubeCLT_1.20.0\...` (khác cả ổ đĩa lẫn version so với
   những gì `launch.json` cũ ghi). Đã sửa cả 3 đường dẫn trong
   `launch.json` sang `C:\ST\STM32CubeCLT_1.20.0\...`.
   **CHƯA XÁC NHẬN** người dùng đã thử debug lại sau khi sửa
   `launch.json` — đây là việc đầu tiên cần làm ở phiên tiếp theo trước
   khi điều tra tiếp bug USB, vì bug USB thật sự chưa hề được debug
   bằng breakpoint/step qua GDB — mọi phân tích trước đó (kể cả giả
   thuyết VBUS sensing đã bác bỏ) đều chỉ dựa trên ĐỌC CODE, chưa có
   dữ liệu debug thật (giá trị thanh ghi, giá trị biến tại đúng thời
   điểm enum fail).

**Đề xuất hướng debug tiếp theo** (chưa làm, để phiên sau/người dùng
quyết định thứ tự): đặt breakpoint tại `tud_mount_cb()`, và tại
`HAL_PCD_MspInit()`/`MX_USB_PCD_Init()` để so sánh giá trị thanh ghi
PCD giữa kịch bản #1 (thành công) và #2 (fail) — đặc biệt các bit liên
quan bus-reset detection trong `USB_DRD_FS` peripheral, vì triệu chứng
#3 (reset khi USB đã cắm sẵn → OK) gợi ý mạnh rằng vấn đề nằm ở TRẠNG
THÁI phần cứng lúc `tusb_init()` chạy, không phải ở cấu hình tĩnh (nếu
là cấu hình sai, kịch bản #3 cũng phải fail vì cấu hình y hệt #2).
...
- **Bài học mới, phiên debug USB enum:** Claude đưa ra giả thuyết
  `vbus_sensing_enable = DISABLE` dựa trên việc nó khớp hợp lý với toàn
  bộ 4 triệu chứng quan sát được (xem mục 5.5) — nhưng người dùng bác bỏ
  ngay bằng 1 câu kinh nghiệm thực tế duy nhất ("sản phẩm trước cấu hình
  y hệt vẫn chạy tốt"). **Khớp lý thuyết với triệu chứng không đủ để kết
  luận nguyên nhân** khi có sẵn dữ liệu đối chứng thực tế (sản phẩm
  tương tự đã chạy ổn) — nên hỏi "đã từng gặp/từng làm cấu hình tương tự
  chưa" SỚM HƠN trong quá trình chẩn đoán, trước khi đầu tư thời gian
  đọc sâu datasheet/tài liệu cho 1 giả thuyết, không phải sau khi đã
  trình bày giả thuyết đầy đủ.
- **Bài học quy trình debug bằng GDB/hardware thật:** khi người dùng
  chuyển sang debug bằng ST-LINK GDB (`.vscode/launch.json`), phần lớn
  lỗi ban đầu KHÔNG PHẢI liên quan tới bug đang điều tra (USB enum) mà
  là lỗi môi trường debug thuần tuý (đường dẫn toolchain sai ổ đĩa/
  version, thư mục build output thiếu 1 cấp). Đã xảy ra 2 lớp lỗi tuần
  tự: (1) GDB server tự đóng kết nối ngay khi khởi động
  (`ECONNRESET`) — hoá ra không phải lỗi thật, chỉ là log verbose chưa
  đủ chi tiết để thấy lỗi thật bên dưới; (2) bật `showDevDebugOutput`
  (ngầm định qua việc người dùng chạy debug và xem Debug Console) mới
  lộ ra lỗi thật: `arm-none-eabi-gdb.exe ENOENT` vì `launch.json` trỏ
  sai ổ đĩa (`D:` không tồn tại, thật ra là `C:\ST\STM32CubeCLT_1.20.0\`).
  Cách xác nhận nhanh nhất, hiệu quả nhất: yêu cầu người dùng chạy
  `Get-Command <tool>.exe | Select-Object -ExpandProperty Source` trong
  PowerShell — cho ngay đường dẫn thật đang có trong PATH, không cần
  đoán qua nhiều vòng hỏi-đáp "đường dẫn này có tồn tại không".